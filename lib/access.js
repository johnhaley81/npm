...
function errCb (e, cb) {
  // AssertionErrors are validation failures coming from
  // npm-registry-client. _probably_ :)
...
function parseParams (cmd, args, cb) {
  ...
  getPackage(args.shift(), function (err, pkg) {
    ...
    if (cmd === 'ls-collaborators') {
      params.scope = args.shift()
    }
    if (!params.scope &&
        (cmd === 'ls-collaborators' || cmd === 'ls-packages')) {
